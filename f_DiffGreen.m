function [Gm]=f_DiffGreen(t,L,D,dim)
% function [Gm]=f_DiffGreen(t,L,D,dim)
% Gives the 1D, 2D, or 3D Green's function 
% response for diffusion, Gm, as a function of time, t at distance L (i.e. 
...
if dim == 1
   ...
   %%%%%% 1D %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % [([1/(2(pi D t]^(1/2)) * [exp(-(L^2)/(4 D t)]  
   Gm=1./(2*(pi.*D.*t).^(1/2)).*exp(-(L).^2./(4*D.*t));
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

elseif dim == 2

   %%%%%% 2D %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % [ [1/(4(pi D t]^(2/2)) * [exp(-(L^2)/(4 D t)]  
   Gm=1./(4*(pi.*D.*t).^(2/2)).*exp(-(L).^2./(4*D.*t));
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

elseif dim == 3

   %%%%%% 3D %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % [[1/(8(pi D t]^(3/2)) * [exp(-(L^2)/(4 D t)]  
   Gm=1./(8*(pi.*D.*t).^(3/2)).*exp(-(L).^2./(4*D.*t));
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
